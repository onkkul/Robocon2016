...
class IRSENSOR
{
  ((unsigned char[]) {2, 4, 5},NUM_SENSORS, TIMEOUT, EMITTER_PIN); 
}
...
void setup()
{
  ...
  line_position = IRSENSOR::readline(sensorValues);
  ...
  {
    Serial.print(IRSENSOR.calibratedMinimumOn[i]);
    Serial.print(' ');
  }
  Serial.println();
   ...
   for (int i = 0; i < NUM_SENSORS; i++)
  {
    Serial.print(IRSENSOR.calibratedMaximumOn[i]);
    Serial.print(' ');
  }
  Serial.println();
  Serial.println();
...
void loop()
{
  unsigned int line_position = IRSENSOR::readline(sensorValues);
...
void follow_line()  //follow the line
{
  ...
  while(1)
  {
    ...
    else if(sensorValues[0] > 200 || sensorValues[2] > 200)
    {
      return;
    }
...
void MazeSolve()
{
    ...
    while(1)
  {
    ...
    {
      line_position = IRSENSOR::readline(sensorValues);
      if(sensorValues[2] > 200 || sensorValues[1] > 200 && sensorValues[0] 
      < 200 && sensorValues[2] < 200)
      break;
      delay(50);
    }
